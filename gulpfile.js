...
gulp.task('preserve-services', function () {
  return gulp.src(['_src/service/data.json'])
      .pipe(concat('data.json'))
      .pipe(gulp.dest('public/service'));
});
...
gulp.task('copy-data-dist', function(done) {
  gulp.src(config.data.src)
    .pipe(plumber({errorHandler: notify.onError('Error: <%= error.message %>')}))
    .pipe(gulp.dest(config.data.dist));
  done();
...
gulp.task('default', gulp.series(['clean','html', 'styles','scripts', 'images', 'icons', 'copy-data'], function(done) {
  browserSync.init({
    server : {
      baseDir: './public/'
    }
...
gulp.task('docs', gulp.series(['clean-dist','html-dist','styles-dist','scripts-dist', 'images-dist', 'icons-dist', 'copy-data-dist'], function(done) {
